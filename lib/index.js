...
async function isSudo(userId) {
    try {
        ...
        // Check each sudo entry
        return data.sudo.some(sudoJid => {
            const sudoNum = sudoJid.split('@')[0].split(':')[0];
            return sudoNum === userNum;
        });
    } catch (error) {
...
async function addSudo(userJid) {
    try {
        ...
        // Normalize JID to standard format (number@s.whatsapp.net)
        const userNum = userJid.split('@')[0].split(':')[0];
        const normalizedJid = userNum + '@s.whatsapp.net';

        // Check if already exists (by number)
        const exists = data.sudo.some(sudoJid => {
            const sudoNum = sudoJid.split('@')[0].split(':')[0];
            return sudoNum === userNum;
        });
...
async function removeSudo(userJid) {
    try {
        ...
        // Find and remove by number
        const initialLength = data.sudo.length;
        data.sudo = data.sudo.filter(sudoJid => {
            const sudoNum = sudoJid.split('@')[0].split(':')[0];
            return sudoNum !== userNum;
        });

        const removed = data.sudo.length < initialLength;
        if (removed) {
            saveUserGroupData(data);
        }
        return removed;
