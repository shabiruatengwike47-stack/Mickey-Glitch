 ...
 * @param {Function} originalGroupMetadata - Original bound groupMetadata function
 * @param {string} chatId - Chat/Group ID
 * @param {boolean} forceRefresh - Force refresh cache
 * @returns {Promise<Object>} - Group metadata
 */
async function getGroupMetadataWithCache(originalGroupMetadata, chatId, forceRefresh = false) {
    try {
        // Check cache first
        ...
        // Make the request using ORIGINAL function (not the wrapped one)
        lastRequestTime = Date.now();
        const metadata = await originalGroupMetadata(chatId);
